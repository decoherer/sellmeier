...
def pulsewidth(λ,Δλ,transformlimited=1): # returns pulse width Δt in ns for λ,dλ in nm
    if transformlimited:
...
def sincfit(w,plotit=False):
    def sincsqrfunc(x,x0,y0,y1,fwhm):
        sinchwhm = 1.39155737825151004649626429454656317830085754394531250 # print 0.5==sinc(sinchwhm)**2
        def sinc(x): return np.sinc(x/np.pi)
        return y0+y1*sinc(2*sinchwhm*(x-x0)/fwhm)**2
    guess = (w.xmax(),w.min(),w.max(),2) # print(guess)
    popt, pcov = scipy.optimize.curve_fit(sincsqrfunc, w.x, w.y, guess) # 
    print('x0,y0,y1,fwhm:',popt)
    x0,y0,y1,fwhm = popt # print((f(x0-fwhm/2)-y0)/y1,(f(x0+fwhm/2)-y0)/y1)
    fx = np.linspace(min(w.x),max(w.x),1001)
    from wave import Wave
    f = Wave(sincsqrfunc(fx,*popt),fx).rename(f'fit, FWHM={fwhm:.2f}nm')
    if plotit: Wave.plots(w.rename('measured'),f,xlabel='wavelength (nm)',ylabel='power (µW)',m=['o',''])
    return fwhm
...
def fabryperotTvsλ(λ0=1064,d=1,n=1.5,F=40,fsrs=20,plot=False): # λ0 in nm, d in mm
    ...
    if plot: w.plot()
    return w
def EOindex(λ,sell='ktpz',temp=20,E=1,low=True): # index change corresponding to Efield in kV/mm # KTP from BierleinVanherzeele89 # LN from www.redoptronics.com/LiNbO3-crystal-electro-optical.html
    ...
    if low:
        r = {'ktpz':36.3,'ktpy':15.7,'ktpx':9.5,'lnz':32,'lny':10,'lnx':10}[sell]
    else:
        r = {'ktpz':35,'ktpy':13.8,'ktpx':8.8,'lnz':32,'lny':8.6,'lnx':8.6}[sell]
    return 0.5 * index(λ,temp=temp,sell=sell)**3 * 1e-12*r * 1e6*E
def groupindex(λ,sell,temp=20,Δλ=0.1):
    Δn = index(λ+Δλ/2,sell,temp=temp) - index(λ-Δλ/2,sell,temp=temp)
...
def shggroupvelocitydelaytime(λ,sell,temp=20,L=10): # L in mm, delay in seconds
...
def sinc(x):
    #return np.where(0==x, 0, np.sin(x)/x)
    return np.sinc(x/pi) # doesn't work with Wave
    # from scipy.optimize import root,brentq
    # def f(x): return sinc(x)**2 - 0.5
    # a,b = brentq(f,0,2,xtol=1e-99),brentq(f,-2,0,xtol=1e-99); print(' {0:.99f}'.format(a)); print('{0:.99f}'.format(b)); print(f(a),f(b)); print(root(f,1).x[0])
    # 1.391557378251510046496264294546563178300857543945312500000000000000000000000000000000000000000000000
def gauss(x,sigma=1,fwhm=None):
    if fwhm is not None: return np.exp( -4*log(2)*x**2/fwhm**2 )
    return np.exp( -x**2 / sigma**2 / 2 )
def defaultwavelengths(w1,w2,w3=None):
...
def polingperiodktpwg(w1,w2,temp,npy,npz,Type='zzz',qpmargs=None):
    w1,w2,w3 = defaultwavelengths(w1,w2)
    ...
    return 1/(kwg - a*knp) # TODO: figure out what depth waveguide this corresponds to
def polingperiod(w1=None,w2=None,sell='ktpwg',Type='zzz',temp=20,w3=None,np1=None,np2=None,qpmargs=None,
    Λ=None,getargs=False,debug=False,quiet=True,returntuple=False):
    ...
    if Λ is not None:
        ...
        assert qpmargs is None, 'not implemented'
        if w1 is None: # 
            if not quiet: print('solving for w1')
            def f(x): return 1/polingperiod(x,w2,sell,Type,temp,np1=np1,np2=np2) - 1/Λ
        elif w2 is None: # 
            if not quiet: print('solving for w2')
            def f(x): return 1/polingperiod(w1,x,sell,Type,temp,np1=np1,np2=np2) - 1/Λ
        elif temp is None: # 
            if not quiet: print('solving for temp')
            def f(x): return 1/polingperiod(w1,w2,sell,Type,x,np1=np1,np2=np2) - 1/Λ
            wx = np.linspace(-100,500,301)
        elif np1 is None: # 
            if not quiet: print('solving for np1')
            assert 'ktpwg'==sell
            def f(x): return 1/polingperiod(w1,w2,sell,Type,temp,np1=x,np2=np2) - 1/Λ
        elif np2 is None: # 
            if not quiet: print('solving for np2')
            assert 'ktpwg'==sell
            def f(x): return 1/polingperiod(w1,w2,sell,Type,temp,np1=np1,np2=x) - 1/Λ
        else:
        ...
        if not 1==len(zero_crossings) and returntuple: return values
        return values[0]
    w1,w2,w3 = defaultwavelengths(w1,w2,w3)
    np2 = np2 if np2 is not None else np1
    if getargs:
        return {'w1':w1,'w2':w2,'w3':w3,'temp':temp,'sell':sell,'Type':Type,'np1':np1,'np2':np2}
    if np1 is not None or qpmargs is not None:
        if 'lnwg'==sell:
            import modes
            return modes.qpm.Λ(w1,w2,temp=temp,sell='ln',Type=Type,verbose=False,**qpmargs)
        assert 'ktpwg'==sell
        return polingperiodktpwg(w1,w2,temp,np1,np2,Type,qpmargs)
    if 0==Type:
    ...
    if 2==Type or 'yzy'==Type or 'ii'==Type:
...
def expansion(temp,sell):
    if 'ln' in sell and sell[-1] in 'xy':
        return 5e-6     ## ranges from 2e-6 to 7e-6, see Browder77,Howlander06
    if 'ln' in sell:
...
def sellmeier3(x,a,b,c,d,e,f):
    return sqrt(1+a/(1-b/x**2)+c/(1-d/x**2)+e/(1-f/x**2))
def sellmeier4(x,a,b,c,d,e,f):
    return sqrt(a+b/(1-c/x**2)+e/(1-f/x**2)-d*x**2)
def sellmeier6(x,a,b,c,d,e,pz,qz):
    return sqrt(a+b/(1-c/x**pz)+d/(1-e/x**qz))
...
def sellmeier10(x,a,b,c,e,f,g):
    return sqrt(a+b/(x**2-c**2)+e/(x**2-f**2)-g*x**2)
...
def index(w,sell,temp=20): # x is wavelength in nm
    np.warnings.filterwarnings('ignore')
    if not np.all(np.logical_or(np.isnan(w),np.logical_and(100<np.abs(w),np.abs(w)<10000))):
        # raise ValueError('Wavelength out of range, w = %g nm' % w)
        print('Wavelength out of range',w)
    x = np.abs(w)/1000.  # change x to um
    temp = np.array(temp) # list becomes np array
    n = None
    if sell=="pm850z" or sell=="pm850":
        n = index(x*1000,"sio2",temp)
    if sell=="pm850y" or sell=="pm850x":
        n = index(x*1000,"pm850z",temp) - 0.00035*2.25/1.1674743331937738 # sign might be wrong
    if sell=="michelsonz":
        n = 2
    ...
    if sell=="lnwg" or sell=="lnwgz":
        n = index(x*1000,"ln",temp) + interp(x*1000,dnzlnx,dnzlny)
    if sell=="lnwgy" or sell=="lnwgx":
        assert 0, 'no type II lnwg'
    if sell=="mglnwg" or sell=="mglnwgz":
        n = index(x*1000,"mgln",temp) + interp(x*1000,dnzlnx,dnzlny)
    if sell=="mglnwgy" or sell=="mglnwgx":
        assert 0, 'no type II mglnwg'
    if sell=="mglnridgewg" or sell=="mglnridgewgz":
        n = index(x*1000,"mgln",temp) + nzmglnridge(x*1000)
    if sell=="mglnhalfridgewg" or sell=="mglnhalfridgewgz":
        n = index(x*1000,"mgln",temp) + nzmglnhalfridge(x*1000)
    if sell=="lnridgewg" or sell=="lnridgewgz":
        n = index(x*1000,"ln",temp) + nzmglnridge(x*1000)
    if sell=="lnhalfridgewg" or sell=="lnhalfridgewgz":
        n = index(x*1000,"ln",temp) + nzmglnhalfridge(x*1000)
    if sell=="mglnridgewgy":
        n = index(x*1000,"mglny",temp) + nzmglnridge(x*1000)
    if sell=="mglnhalfridgewgy":
        n = index(x*1000,"mglny",temp) + nzmglnhalfridge(x*1000)
    if sell=="lnridgewgy":
        n = index(x*1000,"lny",temp) + nzmglnridge(x*1000)
    if sell=="lnhalfridgewgy":
        n = index(x*1000,"lny",temp) + nzmglnhalfridge(x*1000)
    if sell=="gaymglnwg" or sell=="gaymglnwgz":
        n = index(x*1000,"gaymgln",temp) + interp(x*1000,dnzlnx,dnzlny)
    if sell=="ktpwg" or sell=="ktpwgz":
        n = index(x*1000,"ktp",temp) + interp(x*1000,dnzx,dnz)
    if sell=="ktpwgy":
        n = index(x*1000,"ktpy",temp) + interp(x*1000,dnyx,dny)
    if sell=="ktpwgx":
        n = index(x*1000,"ktpx",temp) + interp(x*1000,dnyx,dny)
    if sell=="ktphalfwg" or sell=="ktphalfwgz":
        n = index(x*1000,"ktp",temp) + 0.5*interp(x*1000,dnzx,dnz)
    if sell=="ktphalfwgy":
        n = index(x*1000,"ktpy",temp) + 0.5*interp(x*1000,dnyx,dny)
    if sell=="ktphalfwgx":
        n = index(x*1000,"ktpx",temp) + 0.5*interp(x*1000,dnyx,dny)
    if sell=="ktpcalwg" or sell=="ktpcalwgz":
        n = index(x*1000,"ktpz",temp) + sellmeier16(x*1000,26.7947e-3,-1.09737e-5,2.29268e-9,2.24565e-2,44.62477)
    ...
    if sell=="ktpcalwgy":
        n = index(x*1000,"ktpy",temp) + sellmeier16(x*1000,29.0816e-3,-6.5850e-6,2.13894e-9,9.60547e-3,39.20047)
    ...
    if sell=="ln" or sell=="lnz" or sell=="cln" or sell=="clnz":
        ...
        n = n + index(1000*x,"gaymglnz",temp) - index(1000*x,"gaymglnz",20) ## using MgLN for temperature dependence
    if sell=="lnx" or sell=="lny" or sell=="clnx" or sell=="clny":
        ...
        n = n + index(1000*x,"gaymglny",temp) - index(1000*x,"gaymglny",20) ## using MgLN for temperature dependence
    ...
    if sell=="zelmgln" or sell=="zelmglnz" or sell=="lenmgln":
        ...
        n = n + index(1000*x,"gaymglnz",temp) - index(1000*x,"gaymglnz",20)
    if sell=="zelmglnx" or sell=="zelmglny":
        ...
        n = n + index(1000*x,"gaymglny",temp) - index(1000*x,"gaymglny",20)
    if sell=="yam":
    ...
    if sell=="lnsurfx":  ## sqrt(a+b/(x**2-c)-d*x**2)
        n = sellmeier2(x, 5.063e-3, 1.294e-3, 0.217**2, 0 )  ##Lenzini15 surface delta-n wg no
        assert 20==temp, "lnsurfx temperature dependence of ln not defined"
    if sell=="lnsurfz":  ## sqrt(a+b/(x**2-c)-d*x**2)
        n = sellmeier2(x, 4.646e-3, 9.632e-4, 0.272**2, 0 )  ##Lenzini15 surface delta-n wg ne
    ...
    if sell=="si" or sell=="siz" or sell=="siy":
        ...
        assert np.all(1.357<=x), 'silicon sellmeier not defined: '+str(x)
    ...
    if not np.all(np.logical_or(np.isnan(n),n>=1)) and not sell in ['ktpsurfz','ktpsurfy','lnsurfz','lnsurfx','michelsony','michelsonx']:
        # raise ValueError('index is less than one for '+str(w)+'~'+str(temp)+'~'+sell)
        print('warning, index is less than one for '+str(w)+'~'+str(temp)+'~'+sell)
    return n
...
def jsiplot(w1=1550,w2=1550,temp=20,sell='ktpwg',Type='yzy',lengthinmm=10,dw=None,num=2001,dt=0,
        schmidt=False,minschmidt=False,schmidtres=400,filter=None,filterspectrum=None,
        swapaxes=False,intensity=True,plot=True,plotschmidt=0,qpmargs=None,plotargs={}): # dt = ΔtFWHM in ns
    ...
    def bw(n):
        ...
        return min( 8*max(pulsebw,ppbw), 4*fbw if filterspectrum=='gaussian' else fbw)
    ...
    filterspectrum = gaussian if filterspectrum=='gaussian' else tophat
    def filterfunc(x,y):
        fx = 1 if f1 is None else filterspectrum(x-w1,f1)
        fy = 1 if f2 is None else filterspectrum(y-w2,f2)
        return fx*fy # 1 if filter is None else 1 * filterspectrum(x-w1,f1) * filterspectrum(y-w2,f2)
    def jsa(x,y):
        return sinc( 2*pi * (1/period(x,y)-1/p0) * lengthinmm*1e3/2 )
        # gaussian # return exp(-0.193*( 2*pi * (1/period(x,y)-1/p0) * lengthinmm*1e3/2 )**2)  # sinc(ΔkL/2) ≈ exp(-0.193(ΔkL/2)²) smith,mahou,..walmsley2009 p7
    ...
    if minschmidt:
        def f(a):
            w,dt = jsiplot(w1=w1,w2=w2,temp=temp,sell=sell,Type=Type,lengthinmm=lengthinmm,dw=dw,num=num,dt=a[0],schmidt=False,minschmidt=False,schmidtres=schmidtres,
        ...
        dt = result.x[0]
        return jsiplot(w1=w1,w2=w2,temp=temp,sell=sell,Type=Type,lengthinmm=lengthinmm,dw=dw,num=num,dt=dt,schmidt=True,minschmidt=False,schmidtres=schmidtres,
    ...
    name = f"{Type} {'backward'+str(-w1) if w1<0 else str(w1)}+{'backward'+str(-w2) if w2<0 else str(w2)} {sell} {lengthinmm}mm {dt*1000:g}pspump"
    name = name if filter is None else name+f', {f1,f2}nm filters'
    if schmidt or minschmidt:
        modes = schmidtdecomposition(w,schmidtres)
        K = schmidtnumber(modes)
        print(f'τ = {dt}, K = {K}, purity = {100/K}%, {sum([mode[0]**2 for mode in modes])}')
        if plotschmidt:
...
def schmidtnumber(modes):
    ...
    return 1/sum([m**4 for m in modes])
def schmidtdecomposition(ww,res=100,keep=None,freqspace=True):
    ...
    modes = pyqentangle.continuous_schmidt_decomposition(f,x0,x1,y0,y1, nb_x1=res, nb_x2=res, keep=keep)
    return modes
...
def jsicompare(w1=-1550,w2=810,temp=20,sell='ktpwg',Type='yzy',lengthinmm=10,dw=0.5,num=201,dt=0.02,qpmargs=None): # dt = ?tFWHM in ns
    ...
    return zz
def qpmplot(Λs=None,wp=[405,532,780],temp=20,sell='ktpwg',Type='yzy',np1=None,np2=None,w2onxaxis=1,x0=600,x1=2000):
    def period(x,y):
        return polingperiod(w1=x,w2=y,sell=sell,Type=Type,temp=temp,np1=np1,np2=np2) if not w2onxaxis else polingperiod(w1=y,w2=x,sell=sell,Type=Type,temp=temp)
    x,y = np.linspace(x0,x1,(x1-x0)//5+1),np.linspace(x0,x1,(x1-x0)//5+1)
    yy,xx = np.meshgrid(y,x)
    zz = np.log(np.abs(1./period(xx,yy)))
    zz -= np.nanmin(zz)
    if 'ktp'==sell[:3] and not Type=='zzz': zz = zz**3
    if Type=='zzz': zz = zz**(1/3)
    from wave import Wave2D,Wave
    xlabel,ylabel = (f"λ{'s' if Type[0]==Type[1] else Type[0]} (nm)",f"λ{'i' if Type[0]==Type[1] else Type[1]} (nm)")
    if w2onxaxis: xlabel,ylabel = ylabel,xlabel
    colors = {405:(36864,14592,58880),488:(0,26112,39168),532:(0,39168,0),633:(52224,0,0),775:(65280,26112,26112),780:(65280,26112,26112),976:(65280,32768,32768),1064:(65280,40832,40832),1310:(65280,52224,52224),1550:(65280,58752,58752)}
    ...
    pumpwaves,pumpcolors = [Wave(pumpw2(x,float(w)),x,name=f'{w}nm'+(n==0)*' SFG') for n,w in enumerate(wp)],[rgbtostr(colors[w]) for w in wp]
    colorlines = [{'xdata':[x0,x1], 'ydata':[x0,x1], 'color':'gray','linestyle':(0, (1, 1))}] # + plotargs.pop('lines',[])
    pwaves = [w1vsw2(Λ=p,temp=temp,sell=sell,Type=Type,np1=np1,np2=np2,w2onxaxis=w2onxaxis,x0=x0,x1=x1).rename(f'{abs(p):.1f}µm')
         for p in Λs] if Λs is not None else [] # on w1vsw2 error, check that x0,x1 range is large enough
    from plot import plot
    save = f'phase matching plot, {sell} {Type}'
    import matplotlib.pyplot as plt
    plot(waves=pwaves+pumpwaves,c=['k' for p in pwaves]+pumpcolors,i=['2' for p in pwaves]+['1' for p in pumpwaves],lines=colorlines,
        image=Wave2D(zz,xs=x,ys=y),x=xlabel,y=ylabel,legendtext='',xlim=(x0,x1),ylim=(x0,x1),corner='lower right',
...
def w1vsw2old(w1=None,w2=None,Λ=None,w2onxaxis=True,x0=440,x1=3000,num=101,**ppargs):
    ...
    return ws[0]
def w1vsw2(w1=None,w2=None,Λ=None,w2onxaxis=True,x0=440,x1=3000,num=101,**ppargs):
    # rename to phasematchcurve?
    # option for ±halfmax of phasematchcurve?
    from skimage import measure
    assert w2onxaxis, 'w2onxaxis=False not tested'
    def pp(x,y):
        return polingperiod(w1=x,w2=y,**ppargs) # if w2onxaxis else polingperiod(w1=y,w2=x,**ppargs)
    x,y = np.linspace(x0,x1,num),np.linspace(x0,x1,num)
    yy,xx = np.meshgrid(y,x)
    zz = 1./pp(yy,xx)
    z0 = 1./pp(w1,w2) if w1 is not None else 1/Λ
    contours = measure.find_contours(zz,z0, fully_connected='low', positive_orientation='low')
    from wave import Wave
    wx,wy = Wave(x),Wave(y)
    ws = [Wave( wy(c[:,1]), wx(c[:,0]) ) for c in contours]
...
def phasematchplot(w1=None,w2=None,Λ=None,returnwave=False,w2onxaxis=False,x0=440,x1=3000,num=101,xscale=None,yscale=None,save='out',**kwargs):
    ...
    plt.show()
def polingperiodbandwidths(w1,w2=None,sell='ktpwg',Type='zzz',temp=20,w3=None,np1=None,np2=None,L=10,kind=None,
    ...
    def p1(x): # w2 fixed
        return polingperiod( w1=w1+x, w2=w2, sell=sell, Type=Type, temp=temp, np1=np1, np2=np2 )
    def p2(x): # w1 fixed
        return polingperiod( w1=w1, w2=w2+x, sell=sell, Type=Type, temp=temp, np1=np1, np2=np2 )
    def pshg(x):
        return polingperiod( w1=w1+x, w2=w2+x, sell=sell, Type=Type, temp=temp, np1=np1, np2=np2 )
    def pdc1(x):
        return polingperiod( w1=w1+x, w2=sign(w2)*1/(1/w3-1/np.abs(w1+x)), sell=sell, Type=Type, temp=temp, np1=np1, np2=np2 )
    def pdc2(x):
        return polingperiod( w1=sign(w1)*1/(1/w3-1/np.abs(w2+x)), w2=w2+x, sell=sell, Type=Type, temp=temp, np1=np1, np2=np2 )
    def pdfg1(x): # w1 fixed
        return polingperiod( w1=w1, w2=sign(w2)*1/(1/(w3+x)-1/np.abs(w1)), sell=sell, Type=Type, temp=temp, np1=np1, np2=np2 )
    def pdfg2(x): # w2 fixed
        return polingperiod( w1=sign(w1)*1/(1/(w3+x)-1/np.abs(w2)), w2=w2, sell=sell, Type=Type, temp=temp, np1=np1, np2=np2 )
    def ptemp(x):
        return polingperiod( w1=w1, w2=w2, sell=sell, Type=Type, temp=temp+x, np1=np1, np2=np2 )
    ...
    for k in kinds:
        ...
        if getcurve or plot or debug:
            from wave import Wave
            def g(x):
                return sinc( pi*L*1e3*( 1/pp(x) - 1/pp(0) ))**2
            ux = np.linspace(9*xn,9*xp,10001)
            u = g(ux)
            if not debug:
                uxx = ux + bwparam[k]
                if not quiet: print('  %s bandwidth: %.3f'%(kind,polingperiodbandwidths(w1,w2,sell,Type,temp,L=L,kind=kind,np1=np1,np2=np2)))
                if getcurve: return Wave(index=uxx,data=u)
                Wave(u,uxx).plot(xlabel='λ (nm)' if not 'temp'==k else 'temperature (°C)')
            else:
                u = -1+2*u
...
def pairrate(w1,w2=None,temp=20,sell='ktpwg',Type='zzz',w3=None,np1=None,np2=None,L=10,ηSHG=100,nW=False,pernm=False,units=False): # ηSHG = SHG equivalent con. eff. in %/W/cm²
    bw1 = polingperiodbandwidths(w1,w2=w2,sell=sell,Type=Type,temp=temp,w3=w3,np1=np1,np2=np2,L=L,kind='dc1',quiet=True)
    bw2 = polingperiodbandwidths(w1,w2=w2,sell=sell,Type=Type,temp=temp,w3=w3,np1=np1,np2=np2,L=L,kind='dc2',quiet=True)
    # print(f'Δλ = {bw1}, {bw2} nm')
    rateinHzpermWpernm = (ηSHG/100) * (L/10)**2 * (w1*1e-3)**(-2) * 299792458
    if not nW:
        if pernm:
...
def fixedpumpdc(λp,Λ0=None,λ0=None,λ1=None,temp=20,sell='mglnridgewg',Type='yzy',returnwaves=False):
    ...
    return λ1,λ2
def fixedpumpdc3b(λp,Λ0,λa0=None,λa1=None,λb0=None,λb1=None,temp=20,sell='ktpwg',Type='yzy',np1=None,debug=0):
    from wave import Wave
    ...
    @np.vectorize
    def λi(λs):
        Λ = polingperiod(λs,wix,temp=temp,sell=sell,Type=Type,np1=np1)
...
class Qpm():
    ...
    def __init__(self, **kwargs):
        # w1=None,w2=None,temp=20,sell='ktpwg',Type='zzz',w3=None,np1=None,np2=None,qpmargs=None,Λ=None
        # type 0 sfg period in um, w1,w2 in nm, temp in °C
        aa = ['w1','w2','temp','sell','Type','w3','np1','np2','qpmargs','Λ']
        self.__dict__ = {k:None for k in aa}
        self.__dict__.update(kwargs)
        self.w1,self.w2,self.w3 = defaultwavelengths(self.w1,self.w2,self.w3)
        def setdefault(s,x): # equivalent to self.s = x if self.s is None else self.s
            if getattr(self,s) is None: setattr(self,s,x)
        setdefault('temp',20)
        setdefault('Type','zzz')
        setdefault('np2',self.np1)
        setdefault('Λ',polingperiod(self.w1,self.w2,self.sell,self.Type,self.temp,self.np1,self.np2,self.qpmargs))
    @property
    def λp(self):
        return self.w3
    def period(self):
        return polingperiod(w1=None,w2=None,sell=self.sell,Type=self.Type,temp=self.temp,w3=None,np1=None,np2=None,qpmargs=None)
    def dcwavelengths(self,λp=None,Λ0=None,temp=None,λa0=None,λa1=None,λb0=None,λb1=None,Δλ=200):
        from wave import Wave
        ...
        @np.vectorize
        def λi(λs):
            Λ = polingperiod(λs,wix,self.sell,self.Type,temp,self.np1,self.np2,self.qpmargs)
            return Wave(1/Λ,wix).xaty(1/Λ0)
        w2 = λi(wsx)
        λps = 1/(1/w2+1/wsx)
        # Wave.plots(Wave(wsx,λps),Wave(w2,λps))
    ...
    # o generic plots, e.g. neff vs np1, mfd1x vs period
...
if __name__ == '__main__':
    ...
    test()
    polingperiodbandwidths(1064,1064,'ktpwg',kind='shg',plot=True)
    print(index(215,sell='ktp'))
